...
function commandsTable(commands: TracepackManifest["commands"]): string {
  ...
  return `<table>
  <thead><tr><th>Command</th><th>Exit</th><th>Duration</th><th>Classification</th><th>Evidence</th><th>Output</th></tr></thead>
  <tbody>
    ${commands
      .map(
        (command) => `<tr>
...
function statusLabel(value: string): string {
  const className =
    value.includes("failed") || value === "command_failed"
      ? "bad"
      : value.includes("human") || value.includes("redacted") || value.includes("not_observed")
        ? "warn"
        : value.includes("validation") || value === "observed"
          ? "good"
          : "neutral";
